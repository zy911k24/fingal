...
rho0 = Scalar(RHO_REF, Function(domain) )
rho0.setTaggedValue( 'Base', RHO_BASE)
rho0.setTaggedValue('Seam', RHO_COAL)
rho0.setTaggedValue( 'Goaf', RHO_GOAF)
rho0.setTaggedValue('Core', RHO_MINE)
rho0.expand()
...
for n, v in enumerate([ (50, 170), (100, 80), (180, 0) ]):
    DamageZoneOffset, ResetDamagedZoneSouth = v
    print(n, DamageZoneOffset, ResetDamagedZoneSouth)
    ...
    m3 = clip(interpolate(m, rho0.getFunctionSpace()), minval=0, maxval=1)
    m3.setTaggedValue('Goaf', 0)
    m3.setTaggedValue('Padding', 0)

    rho = rho0 * (1 + m3 * (RAISE_FACTOR_DAMAGED_RESISTIVITY - 1))
    ...
    for A,B,M,N in schedule.tokenIterator():
        ...
        if REL_ERR > 0 :
            ...
            pert = 0.
        u=(potentials_at_stations[iA][iM]-potentials_at_stations[iB][iM]-potentials_at_stations[iA][iN]+potentials_at_stations[iB][iN])
        error = sqrt( (u* REL_ERR)**2 + (noise_floor)**2)
        d = u* (1+pert)
        print(f"{A:d}, {B:d}, {M:d}, {N:d}, {d:g}, {error:g}")
        f.write(f"{A:d}, {B:d}, {M:d}, {N:d}, {d:g}\n")
    f.close()
    print(f"data saved to file {datafile}.")
